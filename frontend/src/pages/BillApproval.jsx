...
const STATUS_CONFIG = {
  master_match:   { label: 'Master Match',    color: 'bg-emerald-100 text-emerald-700 border-emerald-200', icon: Zap },
  ai_suggestion:  { label: 'AI Suggestion',   color: 'bg-sky-100 text-sky-700 border-sky-200',             icon: Bot },
  pending_review: { label: 'Needs Review',    color: 'bg-amber-100 text-amber-700 border-amber-200',       icon: Clock },
  manual_linked:  { label: 'Manually Linked', color: 'bg-violet-100 text-violet-700 border-violet-200',    icon: Link },
  applied:        { label: 'Applied',         color: 'bg-slate-100 text-slate-500 border-slate-200',       icon: CheckSquare },
  no_stock:       { label: 'No Stock Update', color: 'bg-slate-100 text-slate-500 border-slate-200',       icon: MinusCircle },
};
...
function ItemRow({ item }) {
  ...
  return (
    <div className={`p-3 rounded-xl border ${item.conversion_status === 'pending_review' ? 'border-amber-200 bg-amber-50/40' : 'border-slate-100 bg-slate-50'}`}>
      ...
      {item.converted_quantity != null && item.conversion_status !== 'no_stock' && (
        <div className="mt-2 text-[11px] text-emerald-700 font-semibold flex items-center gap-1">
          <Zap size={10} />
          → {item.converted_quantity} servings added to cafeteria
        </div>
      )}
        ...
        <div className="mt-2">
          ...
          {showAI && (
            <div className="mt-2 text-[11px] bg-sky-50 border border-sky-100 rounded-lg p-2 space-y-1 text-slate-700">
              <div><span className="font-bold">Classification:</span> {item.ai_suggestion.classification}</div>
              <div><span className="font-bold">Canonical name:</span> {item.ai_suggestion.suggested_canonical_name}</div>
              {item.ai_suggestion.suggested_units_per_purchase_unit != null && (
                <div><span className="font-bold">Units per purchase:</span> {item.ai_suggestion.suggested_units_per_purchase_unit} {item.ai_suggestion.suggested_storage_unit}</div>
              )}
              <div><span className="font-bold">Confidence:</span> {item.ai_suggestion.confidence}</div>
              <div className="italic text-slate-500">{item.ai_suggestion.reason}</div>
              <div className="text-amber-700 font-bold mt-1">⚠ Pending leadership approval before stock is applied.</div>
            </div>
          )}
        </div>
      )}
...
export default function BillApproval() {
  ...
  async function updateBillStatus(billId, vStatus, aStatus) {
    ...
    setBlockError(null);
    try {
      await api.updateBillStatus(billId, { verification_status: vStatus, approval_status: aStatus });
      await loadBills();
      setSelectedBill(null);
    } catch (e) {
      if (e.message?.includes('Unresolved items') || e.message?.includes('409')) {
        setBlockError('Some items still need conversion review before stock can be applied. Resolve them first.');
      } else {
        alert(e.message);
      }
    } finally {
  ...
  const pendingItems = selectedBill?.bill_items?.filter(i => i.conversion_status === 'pending_review') ?? [];
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
        ...
        <div className="xl:col-span-1 space-y-4">
          ...
          <div className="space-y-3">
            {bills.map((bill) => {
              const unresolvedCount = bill.bill_items?.filter(i => i.conversion_status === 'pending_review').length ?? 0;
              return (
                <button
                  key={bill.id}
                  onClick={() => { setSelectedBill(bill); setBlockError(null); }}
                ...
                >
                  ...
                  <div className="flex items-center gap-2 mt-3">
                    <div className={`text-[10px] font-bold uppercase px-2 py-0.5 rounded-md ${
              ...
              >
                  ...
                  <div className="flex flex-wrap gap-2 items-center">
                    ...
                    <div className="flex flex-col items-end gap-1">
                      <button
                        ...
                        onClick={() => canApprove && updateBillStatus(selectedBill.id, 'Admin Verified', 'Pending Accounts Approval')}
                        disabled={busy || !canApprove}
                        title={!canApprove ? `${pendingItems.length} item(s) need conversion review first` : ''}
                      >
                        <CheckCircle size={16} />
                        {canApprove ? 'Approve & Sync Stock' : `Blocked (${pendingItems.length} unresolved)`}
                      </button>
                    </div>
                  </div>
                </div>
                ...
                {pendingItems.length > 0 && (
                  <div className="mx-6 mt-4 bg-amber-50 border border-amber-200 text-amber-800 text-sm p-3 rounded-xl flex gap-2 items-start">
                    <AlertTriangle size={16} className="shrink-0 mt-0.5" />
                    <span>
                      <strong>{pendingItems.length} item{pendingItems.length > 1 ? 's' : ''} need{pendingItems.length === 1 ? 's' : ''} conversion review.</strong>{' '}
                      Link them to a master rule or classify as supply/equipment/expense before approving.
                    </span>
                  </div>
                )}
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2">
                  ...
                  <div className="p-6 bg-slate-900 flex items-center justify-center min-h-[400px]">
                    {selectedBill.file_url?.match(/\.pdf($|\?)/i) ? (
                      <a href={selectedBill.file_url} target="_blank" rel="noopener noreferrer"
                        className="text-white/70 hover:text-white text-sm flex flex-col items-center gap-2">
                        <FileText size={40} />
                        Open PDF
                      </a>
                    ) : (
                      <img src={selectedBill.file_url} className="max-w-full rounded shadow-2xl cursor-zoom-in" alt="Invoice" />
                    )}
                  </div>
                  ...
                  <div className="p-6 space-y-6 overflow-y-auto max-h-[700px]">
                    <div>
                      <h4 className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-4">Items & Conversion</h4>
                      <div className="space-y-3">
                    ...
                    <div className="pt-6 border-t border-slate-100 space-y-3">
                      <div className="flex justify-between text-sm">
                        ...
                        <span className="font-medium">₹{(Number(selectedBill.grand_total) - Number(selectedBill.delivery_charges) + Number(selectedBill.discount)).toFixed(2)}</span>
                      </div>
                      <div className="flex justify-between text-sm">
