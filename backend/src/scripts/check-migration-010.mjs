...
const SERVICE_KEY  = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InR3bWFkYXVoYXV1eXBpb3pucHVzIiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTc3ODczOTU1MCwiZXhwIjoyMDk0MzE1NTUwfQ._5U3_NVikbUzLkhy8Og6CMx0tL_-HTwu9pI0l8wMZNg';
...
async function runMigration() {
  const mgmtBase = `https://api.supabase.com/v1/projects/twmadauhauuypioznpus/database/query`;
  ...
  const { error: enumErr } = await client.from('requests').select('status').eq('status', 'confirming').limit(1);
  if (enumErr && enumErr.message?.includes('invalid input value for enum')) {
