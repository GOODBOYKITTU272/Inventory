...
// ── Router factory ─────────────────────────────────────────────────────────
// Real imports are the defaults; tests inject mocks via createAdminRouter({...}).

export function createAdminRouter({
  db = supabaseAdmin,
  authMiddleware = requireRole('leadership'),
} = {}) {
  const router = Router();
  ...
  router.get('/users', async (_req, res, next) => {
    try {
      ...
      const { data: usersList, error: uErr } = await db.auth.admin.listUsers({
        page: 1,
        perPage: 200,
      });
      if (uErr) throw uErr;
  ...
  router.post('/users/create', async (req, res, next) => {
    try {
      ...
      // Fail closed when the env var is absent — never fall back to a hardcoded string.
