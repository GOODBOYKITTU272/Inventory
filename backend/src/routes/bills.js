...
router.post('/extract', requireRole('office_boy', 'admin'), async (req, res, next) => {
  try {
    ...
    const { content, model } = isPdf
      ? await fileUrlCompletion({ system: EXTRACTION_SYSTEM, user: 'Extract the details from this PDF vendor bill.', fileUrl: file_url, model: 'gpt-4o' })
      : await visionCompletion({ system: EXTRACTION_SYSTEM, user: 'Extract the details from this bill image.', imageUrl: file_url });
...
router.patch('/:id/status', requireRole('admin', 'leadership', 'finance'), async (req, res, next) => {
  try {
    ...
    if (verification_status === 'Admin Verified' && req.user.role !== 'finance') {
      ...
      for (const item of items) {
        ...
        const converted = item.converted_quantity ?? (master.units_per_purchase_unit != null ? qty * master.units_per_purchase_unit : null);
        ...
        if (match) {
          ...
          await supabaseAdmin
            .from('inventory')
            .update({ current_stock: (inv?.current_stock || 0) + qty, last_updated_by: req.user.id })
            .eq('product_id', match.id);
      ...
      await supabaseAdmin
        .from('bill_uploads')
        .update({ inventory_sync_status: 'complete', inventory_synced_at: new Date().toISOString() })
        .eq('id', bill.id);
    }
...
router.get('/vendor-summary', requireRole('finance', 'leadership'), async (req, res, next) => {
  try {
    ...
    if (month) {
      ...
      q = q.gte('created_at', `${month}-01T00:00:00`).lte('created_at', `${month}-${String(lastDay).padStart(2, '0')}T23:59:59`);
    }
    ...
    for (const bill of bills || []) {
      ...
      if (!vendorMap[vName]) vendorMap[vName] = { vendor_name: vName, bill_count: 0, total_spend: 0, bills: [] };
      ...
      vendorMap[vName].bills.push({
        id: bill.id, invoice_number: bill.invoice_number, bill_date: bill.bill_date,
        grand_total: bill.grand_total, file_url: bill.file_url, created_at: bill.created_at,
        verification_status: bill.verification_status, approval_status: bill.approval_status,
        items: (bill.bill_items || []).map(i => ({
          item_name: i.item_name, quantity: i.quantity, unit: i.unit,
          unit_rate: i.unit_rate, tax: i.tax, total_amount: i.total_amount,
        })),
      });
    }
...
router.get('/conversion-master', requireRole('leadership'), async (req, res, next) => {
  try {
    const { data, error } = await supabaseAdmin
      .from('product_conversion_master')
      .select('*')
...
const MasterSchema = z.object({
  ...
  classification:          z.enum(['direct_menu_stock','ingredient_or_dependency','recipe_stock','internal_supply','equipment_asset','finance_expense','unknown_pending_review']),
...
router.patch('/items/:id/conversion', requireRole('leadership'), async (req, res, next) => {
  try {
    ...
    if (conversion_status && !allowed.includes(conversion_status)) {
      return res.status(400).json({ error: `conversion_status must be one of: ${allowed.join(', ')}` });
    }
