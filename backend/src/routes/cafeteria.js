...
router.patch('/items/:id', requireRole('office_boy', 'facility_manager', 'leadership'), async (req, res, next) => {
  try {
    ...
    const fullAllowed = ['item_name', 'display_name', 'category', 'emoji', 'description', 'available', 'orderable', 'tags', 'sort_order', 'stock_today', 'stock_note', 'stock_servings', 'servings_per_unit', 'unit_label', 'pack_weight', 'supplier', 'sides_option', 'dependencies'];
...
router.get('/self-pickup-status', async (req, res, next) => {
  try {
    const { data, error } = await supabaseAdmin
      .from('v_today_leave')
    ...
    if (data) {
      res.json({
        ...
        message: data.leave_type === 'full_day'
...
router.post('/ob-leave', requireRole('office_boy', 'facility_manager'), async (req, res, next) => {
  try {
    ...
    if (leave_type === 'half_day' && !half_day_slot) {
      return res.status(400).json({ error: 'half_day_slot (morning/afternoon) required for half day leave' });
    }
    ...
    if (leave_date === today) {
      ...
      const { data: empRows } = await supabaseAdmin.from('profiles').select('id').eq('role', 'employee');
      if (empRows?.length) {
        const slotTxt = leave_type === 'full_day' ? 'today' : `this ${half_day_slot}`;
        sendPushToUsers(empRows.map(u => u.id), {
...
router.get('/ob-leave', requireRole('office_boy', 'facility_manager', 'leadership'), async (req, res, next) => {
  try {
...
router.delete('/ob-leave/:id', requireRole('office_boy', 'facility_manager'), async (req, res, next) => {
  try {
